--  Ada_Modbus.Slave - Implementation
...
package body Ada_Modbus.Slave is
   ...
   is
      ...
      CB : Slave_Callbacks renames Config.Callbacks;
      Result : Status;
   begin
      ...
      case FC is
         when FC_Read_Coils =>
            ...
            begin
               ...
               begin
                  Result := CB.Read_Coils (Start_Addr, Quantity, Values);
                  ...
                  --  Build response: FC + ByteCount + CoilStatus
         ...
         when FC_Read_Discrete_Inputs =>
            if CB.Read_Discrete_Inputs = null then
               Build_Exception_Response (FC, Exception_Illegal_Function, Response_PDU, Response_Len);
               return;
            end if;
            ...
            begin
               ...
               declare
                  ...
                  Values   : Coil_Array (0 .. Natural (Quantity) - 1);
               begin
                  Result := CB.Read_Discrete_Inputs (Start_Addr, Quantity, Values);
         ...
         when FC_Read_Holding_Registers =>
            if CB.Read_Holding_Registers = null then
               Build_Exception_Response (FC, Exception_Illegal_Function, Response_PDU, Response_Len);
               return;
            end if;
            ...
            begin
               ...
               begin
                  Result := CB.Read_Holding_Registers (Start_Addr, Quantity, Values);
         ...
         when FC_Read_Input_Registers =>
            if CB.Read_Input_Registers = null then
               Build_Exception_Response (FC, Exception_Illegal_Function, Response_PDU, Response_Len);
               return;
            end if;
            ...
            begin
               ...
               declare
                  ...
                  Byte_Count : constant Natural := Natural (Quantity) * 2;
               begin
                  Result := CB.Read_Input_Registers (Start_Addr, Quantity, Values);
         ...
         when FC_Write_Single_Coil =>
            if CB.Write_Single_Coil = null then
               Build_Exception_Response (FC, Exception_Illegal_Function, Response_PDU, Response_Len);
               return;
            end if;
            ...
            begin
               ...
               Result := CB.Write_Single_Coil (Address, Value);
         ...
         when FC_Write_Single_Register =>
            if CB.Write_Single_Register = null then
               Build_Exception_Response (FC, Exception_Illegal_Function, Response_PDU, Response_Len);
               return;
            end if;
            ...
            declare
               ...
               Value   : constant Register_Value :=
                 From_Big_Endian (Request_PDU (3), Request_PDU (4));
            begin
               Result := CB.Write_Single_Register (Address, Value);
         ...
         when FC_Write_Multiple_Coils =>
            if CB.Write_Multiple_Coils = null then
               Build_Exception_Response (FC, Exception_Illegal_Function, Response_PDU, Response_Len);
               return;
            end if;
            ...
            begin
               ...
               begin
                  ...
                  Result := CB.Write_Multiple_Coils (Start_Addr, Values);
         ...
         when FC_Write_Multiple_Registers =>
            if CB.Write_Multiple_Registers = null then
               Build_Exception_Response (FC, Exception_Illegal_Function, Response_PDU, Response_Len);
               return;
            end if;
            ...
            begin
               ...
               begin
                  ...
                  Result := CB.Write_Multiple_Registers (Start_Addr, Values);
         ...
         when FC_Read_Exception_Status =>
            if CB.Read_Exception_Status = null then
               Build_Exception_Response (FC, Exception_Illegal_Function, Response_PDU, Response_Len);
               return;
            end if;

            --  FC 07 request has no data (just function code)
            declare
               Exception_Status : Byte;
            begin
               Result := CB.Read_Exception_Status (Exception_Status);
               ...
               --  Response: FC(1) + Exception Status(1)
               Response_PDU (0) := Byte (FC);
         ...
         when FC_Diagnostics =>
            ...
            begin
               Result := CB.Diagnostics (Sub_Func, Data_In, Data_Out);
               ...
               --  Response: FC(1) + SubFunction(2) + Data(2)
               Response_PDU (0) := Byte (FC);
         ...
         when FC_Report_Server_Id =>
            if CB.Report_Server_Id = null then
               Build_Exception_Response (FC, Exception_Illegal_Function, Response_PDU, Response_Len);
               return;
            end if;

            --  FC 17 request has no data (just function code)
            declare
               Server_Id     : Byte;
               Run_Indicator : Boolean;
               Add_Data      : Byte_Array (0 .. 249);  --  Max additional data
               Add_Data_Len  : Natural;
            begin
               Add_Data := [others => 0];
               Result := CB.Report_Server_Id (Server_Id, Run_Indicator, Add_Data, Add_Data_Len);
               ...
               --  Response: FC(1) + ByteCount(1) + ServerId(1) + RunIndicator(1) + AddData(n)
               declare
                  Byte_Count : constant Natural := 2 + Add_Data_Len;  --  ServerId + RunIndicator + AddData
               begin
                  Response_PDU (0) := Byte (FC);
                  Response_PDU (1) := Byte (Byte_Count);
         ...
         when FC_Mask_Write_Register =>
            if CB.Mask_Write_Register = null then
               Build_Exception_Response (FC, Exception_Illegal_Function, Response_PDU, Response_Len);
               return;
            end if;
            ...
            begin
               Result := CB.Mask_Write_Register (Address, And_Mask, Or_Mask);
         ...
         when FC_Read_Write_Multiple_Registers =>
            if CB.Read_Write_Registers = null then
               Build_Exception_Response (FC, Exception_Illegal_Function, Response_PDU, Response_Len);
               return;
            end if;
      ...
      case Config.Mode is
         when RTU =>
            ...
            begin
               ...
               if Parse_Result /= Success then
                  return;  --  Invalid frame, no response
               end if;

               --  Check if request is for us
               if Req_Slave /= Config.Unit_Id and then Req_Slave /= Broadcast_Address then
                  return;  --  Not for us
               end if;

               --  Process PDU
               Process_PDU (Config, Request_PDU, Request_Len, Response_PDU, Response_Len);
               ...
               begin
                  Protocol.RTU.Build_Frame
                    (Resp_ADU, Resp_ADU_Len, Config.Unit_Id, Response_PDU, Response_Len);
            ...
            begin
               ...
               begin
                  Protocol.ASCII.Build_Frame
                    (Resp_Frame, Resp_Frame_Len, Config.Unit_Id, Response_PDU, Response_Len);
         ...
         when TCP =>
            ...
            begin
               ...
               --  For TCP, Unit ID 0 typically means "any"
               --  Note: Unit_Id type is 0..247 per Modbus spec, so 255 is not valid
               if Req_Unit /= Config.Unit_Id and then Req_Unit /= 0 then
                  return;
               end if;

               Process_PDU (Config, Request_PDU, Request_Len, Response_PDU, Response_Len);

               --  TCP always sends response (no broadcast concept)
               declare
                  Resp_ADU : Protocol.TCP.ADU_Buffer;
                  Resp_ADU_Len : Natural;
               begin
                  Protocol.TCP.Build_Frame
                    (Resp_ADU, Resp_ADU_Len, Trans_Id, Config.Unit_Id, Response_PDU, Response_Len);
