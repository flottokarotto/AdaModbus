--  Ada_Modbus.CRC16 - Implementation
--  Copyright (c) 2026 Florian Fischer
--  SPDX-License-Identifier: MIT

with Interfaces; use Interfaces;

package body Ada_Modbus.CRC16
  with SPARK_Mode => On
is

   --  CRC-16 lookup table (polynomial 0xA001, reflected)
   CRC_Table : constant array (Unsigned_8) of Unsigned_16 :=
   ...
   function Calculate (Data : Byte_Array) return CRC_Value is
      CRC   : Unsigned_16 := 16#FFFF#;
      Index : Unsigned_8;
   begin
      for B of Data loop
         Index := Unsigned_8 (CRC and 16#FF#) xor Unsigned_8 (B);
         CRC := Shift_Right (CRC, 8) xor CRC_Table (Index);
      end loop;
   ...
   begin
      --  CRC over entire frame including CRC bytes should be 0
   ...
   function Extract (Data : Byte_Array) return CRC_Value is
      Low  : constant Byte := Data (Data'Last - 1);
      High : constant Byte := Data (Data'Last);
   begin
      --  CRC is stored LSB first (Little-Endian)
      return CRC_Value (High) * 256 + CRC_Value (Low);
   end Extract;
   ...
   is
   begin
      --  Append LSB first (Little-Endian)
      Buffer (Buffer'First + Length) := Byte (CRC and 16#FF#);
      Buffer (Buffer'First + Length + 1) := Byte (Shift_Right (Unsigned_16 (CRC), 8));
      Length := Length + 2;
   end Append;
